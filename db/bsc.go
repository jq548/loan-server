...
func (m *MyDb) SelectProvideIncome(address string) ([]model.RewardRecordWithProvideInfo, error) {
	...
	sqls = fmt.Sprintf("SELECT provide_reward_record.hash,provide_reward_record.record_id,provide_reward_record.amount AS income_amount,provide_liquid.amount AS amount,provide_liquid.duration,provide_liquid.start,provide_liquid.status,provide_liquid.create_at,provide_liquid.create_hash,provide_liquid.retrieve_at,provide_liquid.retrieve_hash FROM provide_reward_record LEFT JOIN provide_liquid ON provide_reward_record.record_id=provide_liquid.record_id WHERE provide_reward_record.provider=\"%s\" AND provide_reward_record.type=0;", address)
	tx = m.Db.Raw(sqls).Scan(&record)
	if tx.Error != nil {
		if errors.Is(tx.Error, gorm.ErrRecordNotFound) {
			return nil, nil
	...
	return append(noIncomes, record...), nil
}
